...
bool EvalScriptOld(vector<vector<unsigned char> >& stack, const CScript& script, const CTransaction& txTo, unsigned int nIn, unsigned int flags, int nHashType)
{
    CAutoBN_CTX pctx;
...
bool TransactionSignatureChecker::VerifySignatureOld(const CTransaction& txFrom, const CTransaction& txTo, unsigned int nIn, unsigned int flags, int nHashType)
{
    ...
    return VerifyScriptOld(txin.scriptSig, txout.scriptPubKey, txTo, nIn, flags, nHashType);
}
                  ...
                  unsigned int flags, int nHashType)
{
    ...
    if (!EvalScriptOld(stack, scriptSig, txTo, nIn, flags, nHashType))
        return false;

    stackCopy = stack;

    if (!EvalScriptOld(stack, scriptPubKey, txTo, nIn, flags, nHashType))
        return false;
    if (stack.empty())
        return false;
    ...
    if (scriptPubKey.IsPayToScriptHash())
    {
        ...
        if (!EvalScriptOld(stackCopy, pubKey2, txTo, nIn, flags, nHashType))
            return false;
        if (stackCopy.empty())
            return false;
