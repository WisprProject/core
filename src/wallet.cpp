...
CAmount CWalletTx::GetLockedCredit() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        else if (fMasterNode && vout[i].nValue == 125000 * COIN) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
        }
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex && nTimeFirstKey && (pindex->GetBlockTime() < (nTimeFirstKey - 7200)) && pindex->nHeight <= Params().NEW_PROTOCOLS_STARTHEIGHT())
        ...
        while (pindex) {
            ...
            if (fCheckZWSP && pindex->nHeight >= Params().NEW_PROTOCOLS_STARTHEIGHT()) {
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    //LogPrintf("%s: *** using v1 coin params=%b, using v1 acc params=%b\n", __func__, isV1Coin, chainActive.Height() < Params().NEW_PROTOCOLS_STARTHEIGHT());
