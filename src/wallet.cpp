...
/**
 * Fees smaller than this (in uwsp) are considered zero fee (for transaction creation)
...
bool CWallet::GetMasternodeVinAndKeys(CTxIn &txinRet, CPubKey &pubKeyRet, CKey &keyRet, std::string strTxHash,
...
//            LogPrint("obfuscation", "GetInputObfuscationRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout,
//                     mDenomWtxes[hash].vout[nout].nRounds);
//            return mDenomWtxes[hash].vout[nout].nRounds;
//        }
//
...
//            }
//        }
//        mDenomWtxes[hash].vout[nout].nRounds = fDenomFound ? (nShortest >= 15 ? 16 : nShortest +
//                                                                                     1) // good, we a +1 to the shortest one but only 16 rounds max allowed
//                                                           :
...
int CWallet::ScanForWalletTransactions(CBlockIndex *pindexStart, bool fUpdate) {
    ...
    CBlockIndex *pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            if (fCheckZWSP && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
                ...
                for (auto &m : listMints) {
                    if (IsMyMint(m.GetValue())) {
                        ...
                        if (IsSerialInBlockchain(GetSerialHash(m.GetSerialNumber()), nHeightSpend, txidSpend,
                                                 txSpend)) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const {
    if (fMatureOnly) {
        ...
        for (auto meta : vMints) {
            if (meta.nHeight >= mapMintMaturity.at(meta.denom) || meta.nHeight >= chainActive.Height() ||
                meta.nHeight == 0)
                continue;
...
static void
ApproximateBestSubset(vector <pair<CAmount, pair<const CWalletTx *, unsigned int> >> vValue, const CAmount &nTotalLower,
                      const CAmount &nTargetValue, vector<char> &vfBest, CAmount &nBest, int iterations = 1000) {
...
bool less_then_denom(const COutput &out1, const COutput &out2) {
    ...
    bool found2 = false;
    BOOST_FOREACH(CAmount
    d, obfuScationDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectStakeCoins(std::list <std::unique_ptr<CStakeInput>> &listInputs, CAmount nTargetAmount) {
    ...
    //zWSP
    if (GetBoolArg("-zwspstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() &&
...
bool CWallet::SelectCoins(const CAmount &nTargetValue, set <pair<const CWalletTx *, unsigned int>> &setCoinsRet,
    ...
    if (coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(CAmount
        v, obfuScationDenominations) {
            BOOST_FOREACH(
            const COutput &out, vCoins) {
                if (out.tx->vout[out.i].nValue ==
                    v                                               //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue <
                       nTargetValue + (0.1 * COIN) + 100 //round the amount up to .1 WSP over
                        ) {
                    CTxIn vin = CTxIn(out.tx->GetHash(), out.i);
                    int rounds = GetInputObfuscationRounds(vin);
                    // make sure it's actually anonymized
...
bool
CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &vCoinsRet,
    ...
    const COutput &out, vCoins) {
        ...
        if (nValueRet + out.tx->vout[out.i].nValue <= nValueMax) {
            ...
            //
            // bit 0 - 10000 WSP+1 ( bit on if present )
            // bit 1 - 1000 WSP+1
            // bit 2 - 100 WSP+1
            // bit 3 - 10 WSP+1
            // bit 4 - 1 WSP+1
...
bool
CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector <CTxIn> &setCoinsRet, CAmount &nValueRet,
    ...
    AvailableCoins(vCoins, true, coinControl, false,
                   nObfuscationRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
...
bool CWallet::IsCollateralAmount(CAmount nInputAmount) const {
    return nInputAmount != 0 && nInputAmount % OBFUSCATION_COLLATERAL == 0 &&
           nInputAmount < OBFUSCATION_COLLATERAL * 5 && nInputAmount > OBFUSCATION_COLLATERAL;
}
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx &tx, uint256 hash, bool useIX) {
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 WSP collateral

    CCoinControl *coinControl = NULL;
    bool success = CreateTransaction(vecSend, tx, reservekey, nFeeRet, strFail, coinControl, ALL_COINS, useIX,
                                     (CAmount) 0);
    if (!success) {
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx &tx, uint256 hash, bool useIX) {
    ...
    std::string strFail = "";
    vector <pair<CScript, CAmount>> vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 WSP collateral
...
bool CWallet::CreateTransaction(const vector <pair<CScript, CAmount>> &vecSend,
    ...
    BOOST_FOREACH(
    const PAIRTYPE(CScript, CAmount) &s, vecSend) {
        if (nValue < 0) {
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(
                                "Unable to locate enough funds for this transaction that are not equal 10000 WSP.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _(
                                "Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 WSP.");
                    } else {
                        strFailReason = _(
                    ...
                    if (useIX) {
                        strFailReason += " " +
                                         _("SwiftX requires inputs with at least 6 confirmations, you might need to wait a few minutes and try again.");
                    }
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int)
                pcoin, setCoins) {
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-wispr-address
                    CScript scriptChange;
                ...
                // Fill vin
                BOOST_FOREACH(
                const PAIRTYPE(const CWalletTx*, unsigned int) &coin, setCoins)
...
bool CWallet::CreateCoinStake(const CKeyStore &keystore, unsigned int nBits, int64_t nSearchInterval,
    ...
    if (!txNew.vin[0].scriptSig.IsZerocoinSpend()) {
        ...
        for (const CTxOut &out : txNew.vout) {
            ...
            if (!zwspTracker->HasPubcoinHash(hashPubcoin))
...
string CWallet::PrepareObfuscationDenominate(int minRounds, int maxRounds) {
    ...
    */
    if (minRounds >= 0) {
        if (!SelectCoinsByDenominations(obfuScationPool.sessionDenom, 0.1 * COIN, OBFUSCATION_POOL_MAX, vCoins, vCoins2,
                                        nValueIn, minRounds, maxRounds))
...
void CWallet::AutoZeromint() {
    ...
    CAmount nZerocoinBalance = GetZerocoinBalance(
            false); //false includes both pending and mature zerocoins. Need total balance for this so nothing is overminted.
    ...
    // zWSP are integers > 0, so we can't mint 10% of 9 WSP
    if (nBalance < 10) {
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zWSP\n", nBalance);
        return;
    }

    // Percentage of zWSP we already have
    ...
    if (dPercentage >= nZeromintPercentage) {
        LogPrint("zero",
                 "CWallet::AutoZeromint() @block %ld: percentage of existing zWSP (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                 chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zWSP amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zWSP amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zWSP We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough WSP to mint this denomination
    ...
    if (nMintAmount > 0) {
        ...
        LogPrintf(
                "CWallet::AutoZeromint() @ block %ld: successfully minted %ld zWSP. Current percentage of zWSP: %lf%%\n",
...
void CWallet::AutoCombineDust() {
    ...
    map <CBitcoinAddress, vector<COutput>> mapCoinsByAddress = AvailableCoinsByAddress(true,
                                                                                       nAutoCombineThreshold * COIN);

    //coins are sectioned by address. This combination code only wants to combine inputs that belong to the same address
    for (map < CBitcoinAddress, vector < COutput > > ::iterator it = mapCoinsByAddress.begin(); it !=
                                                                                                mapCoinsByAddress.end();
    it++) {
...
bool CWallet::CreateZWSPOutPut(libzerocoin::CoinDenomination denomination, CTxOut &outMint, CDeterministicMint &dMint) {
    ...
    zwalletMain->GenerateDeterministicZWSP(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction &txNew,
    ...
    while (true) {
        ...
        libzerocoin::CoinDenomination denomination = libzerocoin::AmountToClosestDenomination(nValueRemaining,
                                                                                              nValueRemaining);
        ...
        if (!CreateZWSPOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zwsp output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Wsp
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256 &hashTxOut, CTxIn &newTxIn,
    ...
    bool isV1Coin = libzerocoin::ExtractVersionFromSerial(zerocoinSelected.GetSerialNumber()) <
    ...
    if (!pubCoinSelected.validate()) {
        receipt.SetStatus(_("The selected mint coin is an invalid coin"), ZWSP_INVALID_COIN);
        return false;
    }
    ...
    try {
        libzerocoin::CoinSpend spend(paramsCoin, paramsAccumulator, privateCoin, accumulator, nChecksum, witness,
                                     hashTxOut,
                                     spendType);
        LogPrintf("%s\n", spend.ToString());

        if (!spend.Verify(accumulator)) {
            receipt.SetStatus(_("The new spend coin transaction did not verify"), ZWSP_INVALID_WITNESS);
            //return false;
        ...
        //Add the coin spend into a WISPR transaction
        ...
        try {
            ...
            receipt.SetStatus(_("Failed to deserialize"), ZWSP_BAD_SERIALIZATION);
            return false;
        }
        ...
        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            //Tried to spend an already spent zWSP
...
bool
CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx &wtxNew, CReserveKey &reserveKey,
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zWSP"), nStatus);
        return false;
    }
    ...
    if (vSelectedMints.empty()) {
        ...
        // Select the zWSP mints to use in this spend
    ...
    for (const auto &mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
                nStatus = ZWSP_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zWSP require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            if (nChange) {
                ...
                if (fMintChange) {
                    CAmount nFeeRet = 0;
                    string strFailReason = "";
                    if (!CreateZerocoinMintTransaction(nChange, txNew, vNewMints, &reserveKey, nFeeRet, strFailReason,
                                                       NULL, true)) {
            ...
            //add output to wispr address to the transaction (the actual primary spend taking place)
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(
                        _("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"),
                        ZWSP_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZWSP_SPEND_OKAY); // Everything okay
...
string CWallet::ResetMintZerocoin() {
    ...
    string strResult =
            _("ResetMintZerocoin finished: ") + to_string(updates) + _(" mints updated, ") + to_string(deletions) +
            _(" mints deleted\n");
    return strResult;
}
...
string CWallet::ResetSpentZerocoin() {
    ...
    string strResult =
            _("ResetSpentZerocoin finished: ") + to_string(removed) + _(" unconfirmed transactions removed\n");
    return strResult;
}
...
void CWallet::ReconsiderZerocoins(std::list <CZerocoinMint> &listMintsRestored,
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zwspTracker->UnArchive(hashPubcoin, false)) {
            ...
            zwspTracker->UpdateZerocoinMint(mint);
        }
...
string CWallet::GetUniqueWalletBackupName(bool fzwspAuto) const {
    ...
    return strprintf("wallet%s.dat%s", fzwspAuto ? "-autozwspbackup" : "",
                     DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZWspBackupWallet() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (filesystem::exists(backupPath)) {
            ...
            strNewBackupName = strprintf("wallet-autozwspbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx &wtxNew, CZerocoinSpendReceipt &receipt,
    ...
    int nStatus = ZWSP_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZWSP_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZWSP_ERASE_SPENDS_FAILED);
            }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zwspTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zwspTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZWSP_SPEND_OKAY);  // When we reach this point spending zWSP was successful
...
bool CWallet::GetMintFromStakeHash(const uint256 &hashStake, CZerocoinMint &mint) {
    CMintMeta meta;
    if (!zwspTracker->GetMetaFromStakeHash(hashStake, meta))
...
bool CWallet::IsMyMint(const CBigNum &bnValue) const {
    if (zwspTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum &bnValue, const int &nHeight, const uint256 &txid,
    ...
    CZerocoinMint mint;
    if (zwspTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zwspTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zwspTracker->UpdateState(meta);
    } else {
