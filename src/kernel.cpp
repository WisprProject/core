...
static bool SelectBlockFromCandidates(
    ...
    *pindexSelected = (const CBlockIndex *) 0;
    BOOST_FOREACH(
    const PAIRTYPE(int64_t, uint256) &item, vSortedByTimestamp) {
...
//        nStakeModifier = uint64_t("stakemodifier");
    ...
    int64_t nSelectionIntervalStart =
            (pindexPrev->GetBlockTime() / getIntervalVersion(fTestNet)) * getIntervalVersion(fTestNet) -
            nSelectionInterval;
    ...
    for (int nRound = 0; nRound < min(64, (int) vSortedByTimestamp.size()); nRound++) {
        ...
        if (GetBoolArg("-printstakemodifier", false))
            LogPrintf("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d\n",
                      nRound, DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nSelectionIntervalStop).c_str(), pindex->nHeight,
    ...
    if (GetBoolArg("-printstakemodifier", false)) {
        ...
        BOOST_FOREACH(
        const PAIRTYPE(uint256, const CBlockIndex*) &item, mapSelectedBlocks) {
...
bool CheckStake(const CDataStream &ssUniqueID, CAmount nValueIn, const uint64_t nStakeModifier, const uint256 &bnTarget,
    ...
    LogPrintf("%s: modifier:%d nTimeBlockFrom:%d nTimeTx:%d hash:%s\n", __func__, nStakeModifier, nTimeBlockFrom,
              nTimeTx, hashProofOfStake.GetHex());
...
//                  function,DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nStakeModifierTime).c_str());
        ...
        LogPrintf(
                ...
                sizeof(prevout.n), sizeof(nTimeTx), sizeof(prevout.hash),
...
//    const CTxIn &txin = txPrev.vin[0];
    ...
    LogPrintf("%s : nBits = %08x nTimeTxPrev=%u nPrevout=%u "
              "nTimeTx=%u prevoutHash=%s, prevTxHash=%s \n", __func__, nBits, txPrev.nTime,
              txin.prevout.n, nTimeTx, txin.prevout.hash.ToString(), prevTxHash.ToString());
...
bool CheckProofOfStake(const CBlock block, uint256 &hashProofOfStake, std::unique_ptr <CStakeInput> &stake) {
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        if (!VerifyScript(txin.scriptSig, txPrev.vout[txin.prevout.n].scriptPubKey, SCRIPT_VERIFY_NONE,
...
//    printf("%s Prev pindex height = %ds\n", __func__, pindex->pprev->nHeight);
    if (pindex->nHeight > Params().NEW_PROTOCOLS_STARTHEIGHT()) {
        ...
        if (!CheckStake(txPrev, txin.prevout, tx.nTime, hashProofOfStake, nValueIn, chainActive.Tip(), block.nBits,
                        true)) {
            return error(
                    "CheckProofOfStake() : INFO: old bnStakeModifierV2 check kernel failed on coinstake %s, hashProof=%s \n",
                    tx.GetHash().GetHex(), hashProofOfStake.GetHex());
        }
    }
    return true;
