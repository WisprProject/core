...
static bool AppInitRawTx(int argc, char *argv[]) {
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("Wispr Core wispr-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  wispr-tx [options] <hex-tx> [commands]  " +
                               _("Update hex-encoded wispr transaction") + "\n" +
                               "  wispr-tx [options] -create [commands]   " +
                               _("Create hex-encoded wispr transaction") + "\n" +
                               "\n";
        ...
        strUsage += HelpMessageOpt("-txid",
                                   _("Output only the hex-encoded transaction id of the resultant transaction."));
...
static void MutateTxSign(CMutableTransaction &tx, const string &flagStr) {
    ...
    UniValue prevtxsObj = registers["prevtxs"];
    {
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
            ...
            map <string, UniValue::VType> types = map_list_of("txid", UniValue::VSTR)("vout", UniValue::VNUM)(
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        BOOST_FOREACH(
        const CTransaction &txv, txVariants) {
