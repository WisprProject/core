...
bool static
LookupIntern(const char *pszName, std::vector <CNetAddr> &vIP, unsigned int nMaxSolutions, bool fAllowLookup) {
    vIP.clear();
...
static bool
ConnectThroughProxy(const proxyType &proxy, const std::string strDest, int port, SOCKET &hSocketRet, int nTimeout,
...
bool ConnectSocket(const CService &addrDest, SOCKET &hSocketRet, int nTimeout, bool *outProxyConnectionFailed) {
    ...
    if (GetProxy(addrDest.GetNetwork(), proxy))
        return ConnectThroughProxy(proxy, addrDest.ToStringIP(), addrDest.GetPort(), hSocketRet, nTimeout,
