...
//        pblock->nBits = GetNextWorkRequired(pindexPrev, pblock);
        ...
        if (nSearchTime >= nLastCoinStakeSearchTime) {
            ...
            if (pwallet->CreateCoinStake(*pwallet, pblock->nBits, nSearchTime - nLastCoinStakeSearchTime, txCoinStake, txCoinStake.nTime, pblock)) {
...
//            tx.nTime = GetAdjustedTime();
        ...
        if (!fProofOfStake && nHeight > Params().LAST_POW_BLOCK() && pblock->nTime > Params().StartMasternodePayments()) {
        ...
        }else if (!fProofOfStake && pblock->nTime < Params().StartMasternodePayments()) {
...
//                LogPrintf("Wallet is locked: %s\n", pwallet->IsLocked() ? "true" : "false");
//                LogPrintf("IfMintableCoin: %s\n", fMintableCoins ? "true" : "false");
//                LogPrintf("Wallet balance is higher then zero and lower then or equal to reserve balance: %s\n", (pwallet->GetBalance() > 0 && nReserveBalance >= pwallet->GetBalance()) ? "true" : "false");
//                LogPrintf("MasterNodes are synced: %s\n", masternodeSync.IsSynced() ? "true" : "false");

                MilliSleep(5000);
                if (!fGenerateBitcoins && !fProofOfStake)
                    continue;
            }
//            printf("Staking block created\n");
