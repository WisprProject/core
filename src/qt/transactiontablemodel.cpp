...
public:
    ...
    void updateWallet(const uint256 &hash, int status, bool showTransaction) {
        ...
        qDebug() << "    inModel=" + QString::number(inModel) +
                    ...
                    " showTransaction=" + QString::number(showTransaction) + " derivedStatus=" +
                    QString::number(status);
            ...
            case CT_NEW:
                ...
                if (showTransaction) {
                    ...
                    if (!toInsert.isEmpty()) /* only if something to insert */
                    {
                        ...
                        foreach(
                        const TransactionRecord &rec, toInsert) {
...
TransactionTableModel::TransactionTableModel(CWallet *wallet, WalletModel *parent) : QAbstractTableModel(parent),
                                                                                     ...
                                                                                     fProcessingQueuedTransactions(
                                                                                             false) {
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Address")
...
QString TransactionTableModel::formatTxStatus(const TransactionRecord *wtx) const {
    ...
    switch (wtx->status.status) {
        ...
        case TransactionStatus::Confirming:
            status = tr("Confirming (%1 of %2 recommended confirmations)").arg(wtx->status.depth).arg(
        ...
        case TransactionStatus::Immature:
            status = tr("Immature (%1 confirmations, will be available after %2)").arg(wtx->status.depth).arg(
                    wtx->status.depth + wtx->status.matures_in);
            break;
...
QString TransactionTableModel::formatTxType(const TransactionRecord *wtx) const {
    switch (wtx->type) {
        ...
        case TransactionRecord::ZerocoinMint:
            return tr("Converted WSP to zWSP");
        case TransactionRecord::ZerocoinSpend:
            return tr("Spent zWSP");
        case TransactionRecord::RecvFromZerocoinSpend:
            return tr("Received WSP from zWSP");
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed,
    ...
    switch (role) {
        ...
        case Qt::ForegroundRole:
            ...
            if (rec->status.status == TransactionStatus::Conflicted ||
                rec->status.status == TransactionStatus::NotAccepted) {
                return COLOR_CONFLICTED;
            }
            // Unconfimed or immature
            if ((rec->status.status == TransactionStatus::Unconfirmed) ||
...
public:
    ...
    TransactionNotification(uint256 hash, ChangeType status, bool showTransaction) : hash(hash), status(status),
...
static void
NotifyTransactionChanged(TransactionTableModel *ttm, CWallet *wallet, const uint256 &hash, ChangeType status) {
    // Find transaction in wallet
    std::map<uint256, CWalletTx>::iterator mi = wallet->mapWallet.find(hash);
