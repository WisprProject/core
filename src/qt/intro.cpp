...
public
    ...
    reply(int status, const QString &message, quint64 available);
    ...
    try {
        ...
        if (fs::exists(dataDir)) {
            if (fs::is_directory(dataDir)) {
                ...
                replyStatus = ST_OK;
                replyMessage = tr("Directory already exists. Add %1 if you intend to create a new directory here.").arg(
                        separator);
            } else {
...
bool Intro::pickDataDirectory() {
     ...
     * override -datadir in the wispr.conf file in the default data directory
     * (to be consistent with wisprd behavior)
     */
    if (dataDir != getDefaultDataDirectory())
...
void Intro::on_ellipsisButton_clicked() {
    QString dir = QDir::toNativeSeparators(
            QFileDialog::getExistingDirectory(0, "Choose data directory", ui->dataDirectory->text()));
    if (!dir.isEmpty())
