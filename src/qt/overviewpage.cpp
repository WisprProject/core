...
class TxViewDelegate : public QAbstractItemDelegate {
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::WSP) {
    }
    ...
    inline void paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
        ...
        QRect amountRect(mainRect.left() + xspace, mainRect.top() + ypad, mainRect.width() - xspace - ICON_OFFSET,
                         halfheight);
        QRect addressRect(mainRect.left() + xspace, mainRect.top() + ypad + halfheight, mainRect.width() - xspace,
                          halfheight);
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString &sWSPPercentage,
                                 QString &szWSPPercentage) {
    int nPrecision = 2;
...
void OverviewPage::setBalance(const CAmount &balance, const CAmount &unconfirmedBalance, const CAmount &immatureBalance,
    ...
    CAmount WSPAvailableBalance = balance - immatureBalance;
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance + watchImmatureBalance;
    CAmount nUnlockedBalance =
            nTotalBalance - nLockedBalance - nLockedBalance; // increment nLockedBalance twice because it was added to
    // nTotalBalance above
    // zWSP Balance
    ...
    ui->labelBalance->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, WSPAvailableBalance, false, BitcoinUnits::separatorAlways));
    ui->labelUnconfirmed->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, unconfirmedBalance, false, BitcoinUnits::separatorAlways));
    ui->labelImmature->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, immatureBalance, false, BitcoinUnits::separatorAlways));
    ui->labelLockedBalance->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelWatchLocked->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, nWatchOnlyLockedBalance, false,
    ...
    ui->labelzBalanceUnconfirmed->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, unconfirmedZerocoinBalance, false,
                                            BitcoinUnits::separatorAlways));
    ui->labelzBalanceMature->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, matureZerocoinBalance, false, BitcoinUnits::separatorAlways));
    ui->labelzBalanceImmature->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, immatureZerocoinBalance, false,
                                                                       BitcoinUnits::separatorAlways));

    // Combined labels
    ui->labelBalancez->setText(
            BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, availableTotalBalance, false, BitcoinUnits::separatorAlways));
    ui->labelTotalz->setText(
    ...
    QString automintHelp = tr(
            "Current percentage of zWSP.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    ...
    bool showWatchOnlyWSPAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showWSPPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyWSPPending = watchUnconfBalance != 0;
    bool showWSPLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyWSPLocked = nWatchOnlyLockedBalance != 0;
    ...
    ui->labelWatchAvailable->setVisible(showWSPAvailable && showWatchOnly);
    ...
    ui->labelWatchPending->setVisible(showWSPPending && showWatchOnly);
    ...
    ui->labelWatchLocked->setVisible(showWSPLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel *model) {
    ...
    if (model && model->getOptionsModel()) {
        ...
        setBalance(model->getBalance(), model->getUnconfirmedBalance(), model->getImmatureBalance(),
                   model->getZerocoinBalance(), model->getUnconfirmedZerocoinBalance(),
                   model->getImmatureZerocoinBalance(),
    ...
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        if (currentBalance != -1)
            setBalance(currentBalance, currentUnconfirmedBalance, currentImmatureBalance, currentZerocoinBalance,
                       currentUnconfirmedZerocoinBalance, currentimmatureZerocoinBalance,
