    ...
    if (addressFrame) {
        for (int i = 0; i < ui->addressList->count(); i++) {
            ...
            addressLabel->setText(
                    QApplication::translate("MultisigDialog", strprintf("Address / Key %i:", i + 1).c_str(), 0));
        }
        ...
        for (int i = 0; i < ui->destinationsList->count(); i++) {
            ...
            destinationAddressLabel->setText(
                    QApplication::translate("MultisigDialog", strprintf("%i. Address: ", i + 1).c_str(), 0));
        }
    } else if (keyFrame) {
        ...
        for (int i = 0; i < ui->inputsList->count(); i++) {
            ...
            txInputIdLabel->setText(
                    QApplication::translate("MultisigDialog", strprintf("%i. Tx Hash: ", i + 1).c_str(), 0));
        }
    }
}
    ...
    try {
        ...
        if (validInput) {
            ...
            QString status(strprintf("Transaction has successfully created with a fee of %s.\n"
                                     ...
                                     "Please continue on to sign the tx from this wallet, to access the hex to send to other owners.",
                                     fee).c_str());
...
bool MultisigDialog::createMultisigTransaction(vector <CTxIn> vUserIn, vector <CTxOut> vUserOut, string &feeStringRet,
                                               string &errorRet) {
    try {
        ...
        for (CTxIn in : vUserIn) {
            ...
            if (!fFirst) {
                if (privKey != changePubKey) {
                    throw runtime_error(
                            "Address mismatch! Inputs must originate from the same multisignature address.");
                }
            } else {
                fFirst = false;
    ...
    try {
        ...
        for (CTxIn &txin : tx.vin) {
            ...
            if (!VerifyScript(txin.scriptSig, prevPubKey, STANDARD_SCRIPT_VERIFY_FLAGS,
                              MutableTransactionSignatureChecker(&tx, nIn))) {
                fComplete = false;
            }
            nIn++;
        }
...
void MultisigDialog::on_addAddressButton_clicked() {
    ...
    addressLabel->setText(QApplication::translate("MultisigDialog",
                                                  strprintf("Address / Key %i:", ui->addressList->count() + 1).c_str(),
                                                  0));
...
void MultisigDialog::on_addInputButton_clicked() {
    ...
    txInputIdLabel->setText(
            QApplication::translate("MultisigDialog", strprintf("%i. Tx Hash: ", ui->inputsList->count() + 1).c_str(),
                                    0));
...
void MultisigDialog::on_addDestinationButton_clicked() {
    ...
    destinationAddressLabel->setText(QApplication::translate("MultisigDialog", strprintf("%i. Address: ",
                                                                                         ui->destinationsList->count() +
                                                                                         1).c_str(), 0));
...
void MultisigDialog::on_addPrivKeyButton_clicked() {
    ...
    keyLabel->setText(
            QApplication::translate("MultisigDialog", strprintf("Key %i: ", (ui->keyList->count() + 1)).c_str(), 0));
