...
QString ClientModel::getMasternodeCountString() const {
    ...
    return tr("Total: %1 (IPv4: %2 / IPv6: %3 / Tor: %4 / Unknown: %5)").arg(
            QString::number((int) mnodeman.size())).arg(QString::number((int) ipv4)).arg(
            QString::number((int) ipv6)).arg(QString::number((int) onion)).arg(QString::number((int) nUnknown));
}
...
void ClientModel::updateTimer() {
    ...
    if (cachedNumBlocks != newNumBlocks ||
        ...
        masternodeSync.RequestedMasternodeAttempt != prevAttempt ||
        masternodeSync.RequestedMasternodeAssets != prevAssets) {
