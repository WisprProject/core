...
const char *BIP71_MIMETYPE_PAYMENT = "application/wispr-payment";
const char *BIP71_MIMETYPE_PAYMENTACK = "application/wispr-paymentack";
const char *BIP71_MIMETYPE_PAYMENTREQUEST = "application/wispr-paymentrequest";
...
static void ReportInvalidCertificate(const QSslCertificate &cert) {
    qDebug() << "ReportInvalidCertificate : Payment server found an invalid certificate: "
...
void PaymentServer::ipcParseCommandLine(int argc, char *argv[]) {
    for (int i = 1; i < argc; i++) {
        ...
        // If the wispr: URI contains a payment request, we are not able to detect the
...
PaymentServer::PaymentServer(QObject *parent, bool startLocalServer) : QObject(parent),
    ...
    // on Mac: sent when you click wispr: links
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                                  tr("Cannot start wispr: click-to-pay handler"));
        } else {
...
//
// OSX-specific way of handling wispr: URIs and
...
void PaymentServer::initNetManager() {
    ...
    connect(netManager, SIGNAL(sslErrors(QNetworkReply * ,
    const QList <QSslError>&)),
    this, SLOT(reportSslErrors(QNetworkReply * ,
    const QList <QSslError>&)));
}
...
void PaymentServer::handleURIOrFile(const QString &s) {
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // wispr: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus &request, SendCoinsRecipient &recipient) {
    ...
    foreach(
    const PAIRTYPE(CScript, CAmount) &sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom wispr addresses are not supported
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            emit message(tr("Payment request error"),
                         tr("Requested payment amount of %1 is too small (considered dust).").arg(
                                 BitcoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                         CClientUIInterface::MSG_ERROR);
...
void PaymentServer::handlePaymentACK(const QString &paymentACKMsg) {
    ...
    emit message(tr("Payment acknowledged"), paymentACKMsg,
