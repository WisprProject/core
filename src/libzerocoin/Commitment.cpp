...
//Commitment class
    ...
    Commitment::Commitment(const IntegerGroupParams *p, const CBigNum &bnSerial, const CBigNum &bnRandomness) : params(
            p), contents(bnSerial) {
...
// TODO: get parameters from the commitment group
    CommitmentProofOfKnowledge::CommitmentProofOfKnowledge(const IntegerGroupParams *aParams,
        ...
        CBigNum T1 = this->ap->g.pow_mod(r1, this->ap->modulus).mul_mod((this->ap->h.pow_mod(r2, this->ap->modulus)),
                                                                        this->ap->modulus);
        CBigNum T2 = this->bp->g.pow_mod(r1, this->bp->modulus).mul_mod((this->bp->h.pow_mod(r3, this->bp->modulus)),
                                                                        this->bp->modulus);
    ...
    bool CommitmentProofOfKnowledge::Verify(const CBigNum &A, const CBigNum &B) const {
        ...
        if ((uint32_t)
            this->S1.bitSize() > maxSize ||
            (uint32_t)
        this->S2.bitSize() > maxSize ||
        (uint32_t)
        this->S3.bitSize() > maxSize ||
        this->S1 < CBigNum(0) ||
        this->S2 < CBigNum(0) ||
        this->S3 < CBigNum(0) ||
    ...
    const CBigNum
    CommitmentProofOfKnowledge::calculateChallenge(const CBigNum &a, const CBigNum &b, const CBigNum &commitOne,
