...
///
    ...
    CalculateParams(ZerocoinParams &params, CBigNum N, string aux, uint32_t securityLevel) {
        ...
        for (uint32_t count = 0;
            ...
            params.accumulatorParams.accumulatorBase = constant.pow_mod(CBigNum(2),
...
/// Uses the algorithm described in FIPS 186-3 Appendix A.2.3.
    ...
    CBigNum
    calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, CBigNum modulus, CBigNum groupOrder,
                            uint32_t index) {
        CBigNum result;
