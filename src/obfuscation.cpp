...
void CObfuscationPool::ProcessMessageObfuscation(CNode *pfrom, std::string &strCommand, CDataStream &vRecv) {
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        if (dsq.ready) {
            ...
            BOOST_FOREACH(CObfuscationQueue
            q, vecObfuscationQueue) {
                if (q.vin == dsq.vin) return;
            }

            LogPrint("obfuscation", "dsq last %d last2 %d count %d\n", pmn->nLastDsq,
                     pmn->nLastDsq + mnodeman.size() / 5, mnodeman.nDsqCount);
        ...
        LogPrint("obfuscation", "dssu - state: %i entriesCount: %i accepted: %i error: %s \n", state, entriesCount,
                 accepted, GetMessageByID(errorID));
        ...
        if (success) {
            ...
            RelayStatus(obfuScationPool.sessionID, obfuScationPool.GetState(), obfuScationPool.GetEntriesCount(),
                        MASTERNODE_RESET);
        }
        ...
        case POOL_STATUS_ACCEPTING_ENTRIES:
            if (entriesCount == 0) {
                ...
                if (showingObfuScationMessage % 70 <= 40)
                    return strprintf(_("Submitted following entries to masternode: %u / %d"), entriesCount,
                    ...
                    suffix = "...";
                return strprintf(_("Submitted to masternode, waiting for more entries ( %u / %d ) %s"), entriesCount,
                                 GetMaxPoolTransactions(), suffix);
            }
    ...
    if (state == POOL_STATUS_FINALIZE_TRANSACTION) {
        ...
        if (fMasterNode) {
            ...
            for (unsigned int i = 0; i < entries.size(); i++) {
                BOOST_FOREACH(
                const CTxOut &v, entries[i].vout)
                txNew.vout.push_back(v);

                BOOST_FOREACH(
                const CTxDSIn &s, entries[i].sev)
...
void CObfuscationPool::ChargeFees() {
    ...
    if (state == POOL_STATUS_ACCEPTING_ENTRIES) {
        ...
        const CTransaction &txCollateral, vecSessionCollateral) {
            bool found = false;
            BOOST_FOREACH(
            const CObfuScationEntry &v, entries) {
...
void CObfuscationPool::CheckTimeout() {
    ...
    if (state == POOL_STATUS_SIGNING &&
        GetTimeMillis() - lastTimeChanged >= (OBFUSCATION_SIGNING_TIMEOUT * 1000) + addLagTime) {
...
bool CObfuscationPool::SignatureValid(const CScript &newSig, const CTxIn &newVin) {
    ...
    {
        BOOST_FOREACH(
        const CTxOut &out, e.vout)
        txNew.vout.push_back(out);

        BOOST_FOREACH(
        const CTxDSIn &s, e.sev) {
            txNew.vin.push_back(s);
    ...
    if (found >= 0) { //might have to do this one input at a time?
        ...
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC,
                          MutableTransactionSignatureChecker(&txNew, n))) {
...
bool CObfuscationPool::IsCollateralValid(const CTransaction &txCollateral) {
    ...
    BOOST_FOREACH(
    const CTxIn i, txCollateral.vin) {
        CTransaction tx2;
        uint256 hash;
        if (GetTransaction(i.prevout.hash, tx2, hash, true)) {
            if (tx2.vout.size() > i.prevout.n) {
...
//
bool CObfuscationPool::AddEntry(const std::vector <CTxIn> &newInput, const CAmount &nAmount,
                                const CTransaction &txCollateral, const std::vector <CTxOut> &newOutput, int &errorID) {
...
bool
CObfuscationPool::StatusUpdate(int newState, int newEntriesCount, int newAccepted, int &errorID, int newSessionID) {
    if (fMasterNode) return false;
...
bool CObfuscationPool::SignFinalTransaction(CTransaction &finalTransactionNew, CNode *node) {
    ...
    BOOST_FOREACH(
    const CObfuScationEntry e, entries) {
        BOOST_FOREACH(
        const CTxDSIn s, e.sev) {
            ...
            if (mine >= 0) { //might have to do this one input at a time?
                ...
                if (!SignSignature(keystore, prevPubKey, finalTransaction, mine,
                                   int(SIGHASH_ALL | SIGHASH_ANYONECANPAY))) { // changes scriptSig
                ...
                LogPrint("obfuscation", " -- dss %d %d %s\n", mine, (int) sigs.size(),
                         finalTransaction.vin[mine].scriptSig.ToString());
            }
        }
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun) {
    ...
    if (!sessionFoundMasternode) {
        ...
        if (pwalletMain->GetDenominatedBalance(true) > 0) { //get denominated unconfirmed inputs
            LogPrintf(
                    "DoAutomaticDenominating -- Found unconfirmed denominated outputs, will wait till they confirm to continue.\n");
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH(CObfuscationQueue & dsq, vecObfuscationQueue)
            {
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins,
                                                             vTempCoins2, nValueIn, 0, nZeromintPercentage)) {
...
bool CObfuscationPool::SendRandomPaymentToSelf() {
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, strFail, coinControl,
                                                  ONLY_DENOMINATED);
    if (!success) {
...
bool CObfuScationSigner::SignMessage(std::string strMessage, std::string &errorMessage, vector<unsigned char> &vchSig,
                                     CKey key) {
...
bool CObfuscationQueue::Sign() {
    ...
    std::string strMessage =
            vin.ToString() + boost::lexical_cast<std::string>(nDenom) + boost::lexical_cast<std::string>(time) +
...
void
CObfuscationPool::RelayIn(const std::vector <CTxDSIn> &vin, const int64_t &nAmount, const CTransaction &txCollateral,
                          const std::vector <CTxDSOut> &vout) {
    if (!pSubmittedToMasternode) return;

    std::vector <CTxIn> vin2;
    std::vector <CTxOut> vout2;

    BOOST_FOREACH(CTxDSIn
    in, vin)
    vin2.push_back(in);
...
void
CObfuscationPool::RelayStatus(const int sessionID, const int newState, const int newEntriesCount, const int newAccepted,
                              const int errorID) {
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode * pnode, vNodes)
