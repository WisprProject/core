...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    } else if (strCommand == NetMsgType::DSQUEUE) { //Obfuscation Queue
    ...
    } else if (strCommand == NetMsgType::DSVIN) { //ObfuScation vIn
        int errorID;
    ...
    } else if (strCommand == NetMsgType::DSSIGNFINALTX) { //Obfuscation Sign Final Tx
    ...
    } else if (strCommand == NetMsgType::DSFINALTX) { //Obfuscation Final tx
        if (pfrom->nVersion < ActiveProtocol()) {
            return;
        }
    ...
    } else if (strCommand == NetMsgType::DSCOMPLETE) { //Obfuscation Complete
...
bool CObfuscationPool::CreateDenominated(CAmount nTotalValue)
{
    ...
    // ****** Add denoms ************ /
    for (CAmount v: reverse_iterate(obfuScationDenominations)) {
        int nOutputs = 0;
...
int CObfuscationPool::GetDenominationsByAmounts(std::vector<CAmount>& vecAmount)
{
    ...
    for (CAmount v: reverse_iterate(vecAmount)) {
        CTxOut o(v, e);
        vout1.push_back(o);
    }
