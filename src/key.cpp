...
bool CKey::Sign(const uint256 &hash, std::vector<unsigned char> &vchSig, uint32_t test_case) const {
    ...
    do {
        ...
        nonce += test_case;
        int nSigLen = 72;
        int ret = secp256k1_ecdsa_sign((const unsigned char *) &hash, 32, (unsigned char *) &vchSig[0], &nSigLen,
                                       begin(), (unsigned char *) &nonce);
        nonce = 0;
        if (ret) {
...
bool CKey::VerifyPubKey(const CPubKey &pubkey) const {
    ...
    uint256 hash;
    CHash256().Write((unsigned char *) str.data(), str.size()).Write(rnd, sizeof(rnd)).Finalize(
            (unsigned char *) &hash);
...
bool CKey::SignCompact(const uint256 &hash, std::vector<unsigned char> &vchSig) const {
    ...
    do {
        uint256 nonce;
        prng.Generate((unsigned char *) &nonce, 32);
        int ret = secp256k1_ecdsa_sign_compact((const unsigned char *) &hash, 32, &vchSig[1], begin(),
                                               (unsigned char *) &nonce, &rec);
        nonce = 0;
        if (ret)
            break;
    } while (true);
