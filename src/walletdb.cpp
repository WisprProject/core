...
CAmount CWalletDB::GetAccountCreditDebit(const string &strAccount) {
    ...
    BOOST_FOREACH(
    const CAccountingEntry &entry, entries)
                        ...
                        for (filesystem::directory_iterator dir_iter(pathCustom); dir_iter != end_iter; ++dir_iter) {
                            ...
                            if (filesystem::is_regular_file(dir_iter->status())) {
                                ...
                                if (dir_iter->path().stem().string() == wallet.strWalletFile) {
                                    folderSet.insert(
                                            folder_set_t::value_type(filesystem::last_write_time(dir_iter->path()),
                                                                     *dir_iter));
                                }
                            }
                        }
...
bool CWalletDB::EraseZWSPSeed_deprecated() {
...
bool CWalletDB::WriteZWSPCount(const uint32_t &nCount) {
    return Write(string("dzc"), nCount);
}

bool CWalletDB::ReadZWSPCount(uint32_t &nCount) {
