...
bool BlockToZerocoinMintList(const CBlock &block, std::list <CZerocoinMint> &vMints, bool fFilterInvalid) {
    for (const CTransaction &tx : block.vtx) {
        ...
        for (unsigned int i = 0; i < tx.vout.size(); i++) {
            ...
            uint8_t version = 1;
            CZerocoinMint mint = CZerocoinMint(pubCoin.getDenomination(), pubCoin.getValue(), 0, 0, false, version,
                                               nullptr);
...
CoinSpend TxInToZerocoinSpend(const CTxIn &txin) {
    ...
    libzerocoin::ZerocoinParams *paramsAccumulator = Params().Zerocoin_Params(
            chainActive.Height() < Params().Zerocoin_Block_V2_Start());
    ...
    if (tx.vout.size() > 2) {
        ...
        if (outs > 2 && !tx.IsCoinStake())
            return state.DoS(100,
                             error("CheckZerocoinSpend(): over two non-mint outputs in a zerocoinspend transaction"));
    }
    ...
    if (fZerocoinActive) {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            bool fVerifySignature =
                    !IsInitialBlockDownload() && (GetTime() - chainActive.Tip()->GetBlockTime() < (60 * 60 * 24));
...
bool AcceptToMemoryPool(CTxMemPool &pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zPIV spend tx %s already in block %d",
                                           tx.GetHash().GetHex(), nHeightTx), REJECT_DUPLICATE,
            ...
            if (tx.IsZerocoinMint()) {
                for (auto &out : tx.vout) {
                    ...
                    if (!TxOutToPublicCoin(out, coin, state))
                        return state.Invalid(error("%s: failed final check of zerocoinmint for tx %s", __func__,
                                                   tx.GetHash().GetHex()));
        ...
        if (mapObfuscationBroadcastTxes.count(hash)) {
            ...
            } else if (!tx.IsZerocoinSpend() && GetBoolArg("-relaypriority", true) &&
...
DisconnectBlock(CBlock &block, CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool *pfClean) {
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        // restore inputs
        if (!tx.IsCoinBase() &&
            !tx.IsZerocoinSpend()) { // not coinbases or zerocoinspend because they dont have traditional inputs
...
void RecalculateZPIVSpent() {
    ...
    while (true) {
        ...
        for (auto denom : libzerocoin::zerocoinDenomList) {
            long nDenomAdded = count(pindex->vMintDenominationsInBlock.begin(), pindex->vMintDenominationsInBlock.end(),
                                     denom);
    ...
    if (!listMissingCheckpoints.empty() && chainActive.Height() >= Params().Zerocoin_StartHeight()) {
        ...
        while (pindex) {
            ...
            if (pindex->nAccumulatorCheckpoint != pindex->pprev->nAccumulatorCheckpoint) {
                if (find(listMissingCheckpoints.begin(), listMissingCheckpoints.end(),
                         pindex->nAccumulatorCheckpoint) != listMissingCheckpoints.end()) {
                    ...
                    auto it = find(listMissingCheckpoints.begin(), listMissingCheckpoints.end(),
...
ConnectBlock(const CBlock &block, CValidationState &state, CBlockIndex *pindex, CCoinsViewCache &view, bool fJustCheck,
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                         !((pindex->nHeight == 91842 && pindex->GetBlockHash() ==
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (tx.IsZerocoinMint()) {
                for (auto &out : tx.vout) {
                    ...
                    PublicCoin coin(Params().Zerocoin_Params(false));
                    if (!TxOutToPublicCoin(out, coin, state))
                        return state.DoS(100, error("%s: failed final check of zerocoinmint for tx %s", __func__,
                                                    tx.GetHash().GetHex()));
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_FirstFraudulent() &&
        pindex->nHeight <= Params().Zerocoin_Block_RecalculateAccumulators() + 1)
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput *stake) {
    ...
    if (CZPivStake *zPIV = dynamic_cast<CZPivStake *>(stake)) {
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200,
                                              libzerocoin::AmountToZerocoinDenomination(zPIV->GetValue()));
        if (nChecksum200 != zPIV->GetChecksum())
            return error(
                    ...
                    __func__, zPIV->GetChecksum(), nChecksum200);
    } else {
...
bool static ProcessMessage(CNode *pfrom, string strCommand, CDataStream &vRecv, int64_t nTimeReceived) {
    ...
    if (strCommand == "version") {
        ...
        if (!tx.IsZerocoinSpend() && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs, false, ignoreFees)) {
            ...
            hash, vEraseQueue)EraseOrphanTx(hash);
