...
UniValue getpeerinfo(const UniValue &params, bool fHelp) {
    ...
    const CNodeStats &stats, vstats) {
        ...
        if (fStateStats) {
            ...
            BOOST_FOREACH(int
            height, statestats.vHeightInFlight) {
...
UniValue addnode(const UniValue &params, bool fHelp) {
    ...
    if (fHelp || params.size() != 2 ||
        ...
        throw runtime_error(
                ...
                "\nExamples:\n" +
                HelpExampleCli("addnode", "\"192.168.0.6:51472\" \"onetry\"") +
                HelpExampleRpc("addnode", "\"192.168.0.6:51472\", \"onetry\""));
...
UniValue getaddednodeinfo(const UniValue &params, bool fHelp) {
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "    \"addresses\" : [\n"
                "       {\n"
                "         \"address\" : \"192.168.0.201:51472\",  (string) The wispr server host and port\n"
    ...
    LOCK(cs_vNodes);
    for (list < pair < string, vector < CService > > > ::iterator it = laddedAddreses.begin(); it !=
                                                                                               laddedAddreses.end();
    it++) {
        UniValue obj(UniValue::VOBJ);
...
UniValue getnetworkinfo(const UniValue &params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                ...
                "  ,...\n"
                "  ],\n"
                "  \"relayfee\": x.xxxxxxxx,                (numeric) minimum relay fee for non-free transactions in wispr/kb\n"
    ...
    UniValue localAddresses(UniValue::VARR);
    {
        ...
        BOOST_FOREACH(
        const PAIRTYPE(CNetAddr, LocalServiceInfo) &item, mapLocalHost) {
...
UniValue setban(const UniValue &params, bool fHelp) {
    ...
    if (strCommand == "add") {
        ...
        isSubnet ? CNode::Ban(subNet, BanReasonManuallyAdded, banTime, absolute) : CNode::Ban(netAddr,
