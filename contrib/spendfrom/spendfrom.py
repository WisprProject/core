...
#
# Assumes it will talk to a wisprd or wispr-Qt running
# on localhost.
#
# Depends on jsonrpc
#
...
def determine_db_dir():
    ...
    if platform.system() == "Darwin":
        return os.path.expanduser("~/Library/Application Support/Wispr/")
...
def unlock_wallet(wisprd):
    ...
    if t <= time.time():
        try:
            ...
            wisprd.walletpassphrase(passphrase, 5)
        except:
...
def list_available(wisprd):
    ...
    for info in wisprd.listreceivedbyaddress(0):
        address_to_account[info["address"]] = info["account"]

    unspent = wisprd.listunspent(0)
    for output in unspent:
        # listunspent doesn't give addresses, so:
        rawtx = wisprd.getrawtransaction(output['txid'], 1)
        vout = rawtx["vout"][output['vout']]
        pk = vout["scriptPubKey"]

        # This code only deals with ordinary pay-to-wispr-address
...
def create_tx(wisprd, fromaddresses, toaddress, amount, fee):
    ...
    rawtx = wisprd.createrawtransaction(inputs, outputs)
    signed_rawtx = wisprd.signrawtransaction(rawtx)
...
def compute_amount_in(wisprd, txinfo):
    ...
    for vin in txinfo['vin']:
        in_info = wisprd.getrawtransaction(vin['txid'], 1)
...
def main():
    ...
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of wispr.conf file with RPC username/password (default: %default)")
